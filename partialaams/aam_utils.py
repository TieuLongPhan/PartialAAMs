...
def smiles_to_graph(
    ...
    if ">>" in smiles:
        ...
        g = rdmolfiles.MolFromSmiles(smiles_token[0], sanitize=False)
        h = rdmolfiles.MolFromSmiles(smiles_token[1], sanitize=False)
        Chem.SanitizeMol(g)
        Chem.SanitizeMol(h)
        g = mol_to_graph(g)
        h = mol_to_graph(h)
...
def graph_to_mol(
    ...
    for n, d in G.nodes(data=True):
        ...
        if "hcount" in d:  # Set the number of hydrogen atoms for the atom
            rw_mol.GetAtomWithIdx(idx).SetNumExplicitHs(d["hcount"])
